...
"""
import numpy as np
from simulator_viewComp import PlatformSimulator_ViewComp 
...
def sweep_Q(model, params, k, Q_min, Q_max, n, n_beta=1001):
    ...
    for i, Q in enumerate(Qs):
        sim = PlatformSimulator_ViewComp(Q=Q, k=k, model=model, **params)
...
def sweep_k(model, params, Q, k_min, k_max, n, n_beta=1001):
    ...
    for i, kv in enumerate(ks):
        sim = PlatformSimulator_ViewComp(Q=Q, k=kv, model=model, **params)
        res = _collect(sim, sim.optimize(n_beta))
        for key in KEYS:
            out[key][i] = res[key]
    return ks, out
...
def sweep_alpha(model, params, k, Q, a_min, a_max, n, n_beta=1001):
    ...
    for i, a in enumerate(alphas):
        sim = PlatformSimulator_ViewComp(alpha=a, Q=Q, k=k, model=model, **p)
        res = _collect(sim, sim.optimize(n_beta))
        for key in KEYS:
            out[key][i] = res[key]
    return alphas, out
...
def sweep_delta(model, params, k, Q, d_min, d_max, n, n_beta=1001):
    ...
    for i, d in enumerate(deltas):
        sim = PlatformSimulator_ViewComp(delta=d, Q=Q, k=k, model=model, **p)
        res = _collect(sim, sim.optimize(n_beta))
        for key in KEYS:
            out[key][i] = res[key]
    return deltas, out
