...
def plot_Q_sweep(x, data_by_k, model, filename):
    ...
    panels = [(axes[0,0],'beta_h',r'$\beta_h^*$','Promotion Weight'),
              ...
              (axes[1,0],'uc',r'Creator Welfare $u_c^*$','Creator Welfare'),
              (axes[1,1],'utility',r'Platform Welfare $u_p^*$','Platform Welfare')]
...
def plot_alpha_sweep(x, data_by_k, model, filename):
    _setup()
    fig, axes = plt.subplots(2, 3, figsize=(17, 9))
    panels = [(axes[0,0],'beta_h',r'$\beta_h^*$','Promotion'),
              (axes[0,1],'r',r'$r^*$','Compensation'),
              (axes[0,2],'cs','Consumer Surplus','Consumer Surplus'),
...
def plot_platform_welfare(x, data_by_k, model, filename):
    _setup()
    fig, ax = plt.subplots(figsize=(7, 5))
    for k, d in sorted(data_by_k.items()):
        ax.plot(x, d['utility'], '-', color=_kc(k),
                lw=2, label=f'$k={k}$')
    ax.set_xlabel(r'AI Baseline Quality $Q$')
    ax.set_ylabel(r'$u_p^*$')
    ax.set_title(f'{model}: Platform Welfare')
    ax.grid(True, alpha=0.3, ls='--'); ax.legend()
    fig.tight_layout(); _save(fig, filename); return fig