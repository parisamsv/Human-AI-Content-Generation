...
def check_assumptions(p):
    ...
    if not (Q_hi < t * (1 - delta)):
        problems.append(f"non-saturation fails at Q={Q_hi}: need Q < t(1-delta) = {t*(1-delta):.3f}")
    if not (t > alpha_hi**2 + 1 / (1 - delta)):
        problems.append(f"concavity fails at alpha={alpha_hi}: need t > alpha**2 + 1/(1-delta) = {alpha_hi**2 + 1/(1-delta):.3f}")
